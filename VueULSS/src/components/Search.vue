            ...
            <el-row :gutter="10">
                <el-col :span="6" v-for="(item,index) in items" :key="index">
                    <el-card :body-style="{padding:'0'}">
                        <router-link v-bind:to="'/detail/'+item.spu_id">
...
<script>
    export default {
        ...
        methods:{
            searchItem:function(){
                let url="http://10.214.213.43:9000/product/search?key="+this.key+"&page="+this.currentPage+"&size="+this.pageSize;
                console.log(url);
                fetch(url, {
                    method: 'GET',
                    mode: 'cors',
